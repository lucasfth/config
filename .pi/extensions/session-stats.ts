...
function shortenPath(path: string, maxLen: number = 20): string {
  ...
  const short = path.replace(
    process.env.HOME || "/Users/lucasfreytorreshanson",
    "~",
  );
...
export default function (pi: ExtensionAPI) {
  ...
  function toggleWidget(ctx: ExtensionContext) {
    ...
    if (!widgetVisible) {
      ...
      ctx.ui.notify(
        `Showing ${files.length} modified file${files.length === 1 ? "" : "s"}`,
        "info",
      );
    }
  }
    ...
    footerDisposer = ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          const providerStr = (model as any)?.provider
          ...
          const thinkingLabel =
            currentThinkingLevel === "off" ? "off" : currentThinkingLevel;
          const thinkingStr = theme.fg(
            "muted",
