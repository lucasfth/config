...
### Vision Not Working
If `vision_analyze` fails with "No endpoints found that support image input", the current
model may not support vision. Use OCR as fallback:

```bash
# Install OCR tools (one-time)
...
import pytesseract
import re
img = Image.open('/tmp/findmy.png')
text = pytesseract.image_to_string(img)
# Extract lines with location patterns
for line in text.split('\n'):
    if any(x in line.lower() for x in ['now', 'min ago', 'hr ago', 'km']):
        print(line)
"
```

### Screenshot Errors
- "could not create image from window": Permission not granted - enable Screen Recording in System Settings → Privacy & Security
- Use `screencapture -x` (no sound) for cleaner captures
- Full window capture works better than -w (window-only) which can fail
